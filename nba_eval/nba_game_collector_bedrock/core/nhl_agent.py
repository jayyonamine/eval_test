"""
NHL Game Data Collection Agent
...
class NHLGameCollectionAgent:
    """AI agent that collects NHL game data using Claude via Amazon Bedrock"""
    ...
    def __init__(self, aws_region: str, skill_path: str, aws_access_key: str = None,
        ...
        Args:
            ...
            skill_path: Path to the NHL game collection skill file
    ...
    def _fetch_nhl_games_api(self, target_date: str) -> dict:
        """Fetch NHL games from the official NHL API for a specific date"""
        # NHL API endpoint format: https://api-web.nhle.com/v1/score/YYYY-MM-DD
        ...
        try:
            ...
            print(f"Error fetching NHL API: {e}")
            return None
    ...
    def collect_games(self, target_date: str = None) -> list:
        ...
        # Fetch NHL game data from API
        print(f"Fetching NHL scores from API for {target_date}")
        api_data = self._fetch_nhl_games_api(target_date)

        if not api_data:
            print("Failed to fetch NHL API data")
            return []

        # Construct the prompt with skill instructions and API data
        prompt = f"""I have fetched NHL game data from the official NHL API for {target_date}. Please extract all NHL games from the JSON data below.

NHL API Response:
{json.dumps(api_data, indent=2)}

Using the NHL Game Data Collection Skill guidelines below, convert the API data into the required format.
...
- Convert team codes/abbreviations to full official NHL team names
- Extract arena/location information if available, or use standard arena for that team
- Only include games with "Final" or "OFF" (Final/Offical) status
- If no completed games are found, return an empty array: []

Respond with only the JSON array."""

        # Call Claude via Bedrock to parse the HTML
        request_body = {
...
def run_daily_collection(
    ...
    """
    print(f"Starting NHL game collection for {target_date or 'yesterday'}...")

    # Initialize agent
    # Use path relative to this script's directory
    script_dir = os.path.dirname(os.path.abspath(__file__))
    skill_path = os.path.join(script_dir, "nhl_game_collector_skill.md")
    ...
    writer = NBAGameBigQueryWriter(
        ...
        sport="nhl"  # Enable automatic game_id lookup from forecasts
    )
