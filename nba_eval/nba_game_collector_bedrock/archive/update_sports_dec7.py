"""
Update all sports (NBA, NHL, NFL) through December 7, 2025
"""
...
def main():
    print("=" * 70)
    print("UPDATING ALL SPORTS THROUGH DECEMBER 7, 2025")
    print("=" * 70)
    ...
    # Update NBA
    if nba_last and nba_last < "2025-12-07":
        next_date = (datetime.strptime(nba_last, "%Y-%m-%d") + timedelta(days=1)).strftime("%Y-%m-%d")
        update_sport("NBA", "nba_data", nba_collect, next_date, "2025-12-07")
    else:
        print(f"\nNBA already up to date through 2025-12-07")

    # Update NHL
    if nhl_last and nhl_last < "2025-12-07":
        next_date = (datetime.strptime(nhl_last, "%Y-%m-%d") + timedelta(days=1)).strftime("%Y-%m-%d")
        update_sport("NHL", "nhl_data", nhl_collect, next_date, "2025-12-07")
    else:
        print(f"\nNHL already up to date through 2025-12-07")

    # Update NFL
    if nfl_last and nfl_last < "2025-12-07":
        next_date = (datetime.strptime(nfl_last, "%Y-%m-%d") + timedelta(days=1)).strftime("%Y-%m-%d")
        update_sport("NFL", "nfl_data", nfl_collect, next_date, "2025-12-07")
    else:
        print(f"\nNFL already up to date through 2025-12-07")
